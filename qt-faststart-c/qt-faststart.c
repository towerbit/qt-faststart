    ...
    void *context)
{
    ...
    while (end - pos >= ATOM_PREAMBLE_SIZE) {
        ...
        remaining_size = (uint64_t)(end - pos);
        if (atom.size > remaining_size) {
...
static int update_stco_offsets(update_chunk_offsets_context_t *context, atom_t *atom)
{
    ...
    for (pos = atom->data + 8, end = pos + offset_count * 4;
        ...
        current_offset = (uint32_t)(current_offset + context->moov_atom_size);
...
static void upgrade_stco_atom(upgrade_stco_context_t *context, atom_t *atom)
{
    ...
    for (pos = atom->data + 8, end = pos + offset_count * 4;
        pos < end;
        pos += 4) {
        /* Preserve the original 32-bit wraparound semantics before upgrading to co64. */
        original_offset = BE_32(pos) - (uint32_t)context->original_moov_size;
...
int main(int argc, char *argv[])
{
     ...
     * moov atom */
    if (atom_size > (uint64_t)INT64_MAX - free_size) {
        fprintf(stderr, "file offset too large\n");
        goto error_out;
    }
    {
        int64_t seek_offset = (int64_t)(atom_size + free_size);
        if (fseeko(infile, -seek_offset, SEEK_END)) {
            perror(argv[1]);
            goto error_out;
        }
    }
    ...
    bytes_to_copy = (size_t)((last_offset < (int64_t)COPY_BUFFER_SIZE) ?
        last_offset : (int64_t)COPY_BUFFER_SIZE);
    copy_buffer = malloc(bytes_to_copy);
    if (!copy_buffer) {
        fprintf(stderr, "could not allocate %zu bytes for copy_buffer\n", bytes_to_copy);
        goto error_out;
    }
    printf(" copying rest of file...\n");
    while (last_offset > 0) {
        if (last_offset < (int64_t)bytes_to_copy) {
            bytes_to_copy = (size_t)last_offset;
        }
